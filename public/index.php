...
require __DIR__ . '/../classes/Sms.php';
...
$config['db']['user']   = "root";
$config['db']['pass']   = "root";
$config['db']['dbname'] = "mixpanel";

$config['sms']['key'] 	= "";
$config['sms']['secret']= "";
$config['sms']['phone_from'] = "";
$config['sms']['phone_to'] = "";
$config['sms']['message'] = "New lead";
...
$container['sms'] = function($c) {
    	$sms = $c['settings']['sms'];
	return new NexmoSMS($sms['key'], $sms['secret'], $sms['phone_from']);
};
...
$app->post('/mixpanel', function (Request $request, Response $response, array $args) use ($app) {
	...
	** set to full absolute path
	$json = file_get_contents("data/sample_mixpanel.json");
	*/
	$sms_settings = $this->get('settings')['sms'];
	...
	foreach ($data_db as $value) {
		...
		if( ! $event_mapper->getSmsSentStatus( $value['distinct_id'])['status'] ) {
			$sms_response = $this->sms->send($sms_settings['phone_to'], $sms_settings['message']);
		}
